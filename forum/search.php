<?php
    ...
    preg_match_all('/\[(?>[^\[\]]|(?R))*]/', $search, $tags);
  
  $contentSearch = $search;

  for($i = 0; $i < count($tags[0]); $i++) {
    $contentSearch = str_replace($tags[0][$i], '', $contentSearch);
    $tags[0][$i] = substr($tags[0][$i], 1, -1);
  }
  ...
  <body>
		...
		<div id="report-modal" class="modal">
          ...
          <div id="report-detail-container">
            ...
            <textarea name="report-detail" id="report-detail" placeholder="Elaborate what is wrong with this post..."></textarea>
					...
					<div class="modal-button-holder modal-alert-button-holder">
						<div class="btn btn-default modal-dismiss">Cancel</div>
						<div id="submit-report" class="single-thread-report btn btn-red disabled" data-thread_id="" data-reply_id="" data-post_ver="" data-user_id="0">Report</div>
					</div>
				</div>
			</div>
		</div>
			...
			<div class="modal-content">
				<div class="modal-body">
					...
					<div id="delete-reply-yes-cancel" class="modal-button-holder modal-alert-button-holder">
						...
						<div>
							<div class="btn btn-default modal-dismiss modal-dismiss-btn">Cancel</div>
							<div id="delete-reply-btn" class="btn btn-green" data-reply_id="" data-thread_link="">Yes</div>
						</div>
					</div>
					...
					<div id="confirm-yes-cancel" class="modal-button-holder modal-alert-button-holder">
						...
						<div>
							<div class="btn btn-default modal-dismiss modal-dismiss-btn">Cancel</div>
							<div id="cancel-update" class="btn btn-green confirm-close" data-thread_type="">Yes</div>
						</div>
					</div>
    ...
    <div id="main" class="space-above">
      ...
      <div id="main-inner" class="width-limit thread-layout">
        <div id="search-content" class="forum-header">
          <div id="search-sidebar">
            <i class="fas fa-search fa-lg"></i>
            <input type="text" id="forum-search" placeholder="Search for a keyword or tag..." value="<?php echo htmlspecialchars_decode($search); ?>">
          ...
          <div id="search-result">
            <div>Search results for 
              <div id="searched">
                ...
                ?><div class = "tag"><?php echo $tag; ?></div><?php
                    }

                    if($contentSearch != '')
                      echo htmlspecialchars_decode($contentSearch); 
                  } else
                    echo htmlspecialchars_decode($search); 
                ?><span>”</span>
              </div>
            </div>
          </div>
        ...
        <div id="forum-content-container">
          ...
          <div id="no-thread">
            <i class="fas fa-envelope-open-text fa-8x"></i>
            <h3>No threads found.</h3>
            <a href="submit.php">Be the first to post what you think!</a>
          </div>
          <div id="page-row">
